...
void NewTheme::drawRecentBookCover(GfxRenderer& renderer, Rect rect, const std::vector<RecentBook>& recentBooks,
  ...
  if (hasContinueReading) {
    ...
    if (selected) {
      renderer.fillRoundedRect(coverX - 4, coverY - 4, coverWidth + 8, coverHeight + 8, cornerRadius, Color::LightGray);
    ...
    auto titleLines =
        renderer.wrappedText(UI_12_FONT_ID, book.title.c_str(), textWidth, titleLinesLimit, EpdFontFamily::BOLD);
    auto author = renderer.truncatedText(UI_10_FONT_ID, book.author.c_str(), textWidth);

    int currentY = textY;
    for (const auto& line : titleLines) {
      const int textX =
          rect.x + (rect.width - renderer.getTextWidth(UI_12_FONT_ID, line.c_str(), EpdFontFamily::BOLD)) / 2;
    ...
    if (!book.author.empty()) {
      currentY += 2;
      const int textX =
          rect.x + (rect.width - renderer.getTextWidth(UI_10_FONT_ID, author.c_str(), EpdFontFamily::REGULAR)) / 2;
    ...
    const int textX =
        rect.x + (rect.width - renderer.getTextWidth(UI_12_FONT_ID, noBooksText, EpdFontFamily::REGULAR)) / 2;
    renderer.drawText(UI_12_FONT_ID, textX, coverY + coverHeight + NewMetrics::values.verticalSpacing, noBooksText,
                      true);
...
void NewTheme::drawButtonMenu(GfxRenderer& renderer, Rect rect, int buttonCount, int selectedIndex,
  ...
  for (int i = 0; i < buttonCount; ++i) {
    ...
    if (selected) {
      const int labelY = y + iconSize + 8;
      const int labelHeight = renderer.getLineHeight(SMALL_FONT_ID);

      renderer.fillRoundedRect(x - 20, y - 12, iconSize + 40, (labelY - y) + labelHeight + 12, cornerRadius,
                               Color::LightGray);
    }
    ...
    if (rowIcon != nullptr) {
      ...
      if (iconBitmap != nullptr) {
        ...
        // Get longest word from buttonLabel
        ...
        // If label contains "opds", force label to "OPDS"
        if (label.find("opds") != std::string::npos || label.find("OPDS") != std::string::npos) {
          label = "OPDS";
        } else {
          label = longestWord;
        }
        ...
        // Center text beneath icon
        const int textX = x + (iconSize - labelWidth) / 2;

        // Position text below icon
        const int textY = y + iconSize + 8;
